...
$columns="Fecha de Creacion,Nombre,Folio,Años,Procedencia,Liberado";
...
$fol=$_GET['folio'];
// echo $fol;
$con="SELECT distinct(NOMBRE_COMPLETO),idpaciente,fecha_atencion, id_procemuestra, numero_atencion, sexo 
								...
								if ($hacer==true)
								{
									...
									{*/
										if (count($datoss)==1)
										{
											$rowf1=explode("\"",$rowf);
											$dat= explode("&",$rowf1[1]);
											$seguir_no="SELECT distinct(NOMBRE_COMPLETO),idpaciente,fecha_atencion, 
											id_procemuestra, numero_atencion, sexo,USUARIO_CREACION_DETALLE FROM SISTEMA_ATENCION_EXAMENES 
											WHERE numero_atencion=".$fol;#"EXECUTE SISTEMA_RESULTADOS_WEB '".$dat[0]."'";
											$sk=odbc_exec($conection,$seguir_no) or die ("ERROR : No se puede ejecutar la consulta.");
											//echo $seguir_no;
											while ($rsd=odbc_fetch_array($sk))
											{															
											...
											if ($ser=="S")
											{
												...
												function abrir()
												{
													window.open ("<?php echo "Core/Resultados_r.php?id=".$dat[0];?> ","_self",""); 
												}
												</script>
												</p>
