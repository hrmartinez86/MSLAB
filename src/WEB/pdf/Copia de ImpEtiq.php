...
$sql="SELECT     fecha, hora, numero, numero_registro, rut, usuario_creacion, nombre_usuario, años, NOMBRE_DOCTOR, FechaTomaMuestra, 
			      ...
			      while ($result=odbc_fetch_array($query))
			          {
			        	...
			        	$fx=explode("-",$fecha[0]);
			        	$fecha_ingreso=$fx[2]."/".$fx[1]."/".$fx[0]." ".$result['hora']." Sx:".$result['sexo']." Ed:".$result['años'];
...
if($i<=0){
	$pdf->AddPage();
	$pdf->Code39(17, 5, $a,$folios,$n,$fecha_ingreso,$proc,$i,$e,$m,$opcion[$t-1]);
		...
		$pdf->AddPage();
             if ($opcion[$t-1] == 31 || $opcion[$t-1]==32) 
	    {$pdf->Code39(17, 1, $a,$folios,$n,$fecha_ingreso,$proc,$i,$e,$m,$opcion[$t-1]);}
             else
            {$pdf->Code39(18, 5, $a,$folios,$n,$fecha_ingreso,$proc,$i,$e,$m,$opcion[$t-1]);}
	    $e='';
		}
