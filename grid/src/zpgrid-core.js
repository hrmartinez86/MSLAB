...
Zapatec.Grid.prototype.configure = function (oArg) {
  ...
  this.visualize = true;
  if (
    typeof oConfig.callbackHeaderDisplay == "function" &&
  ...
  if (
    typeof oConfig.callbackCellOnRightClick == "function" ||
...
Zapatec.Grid.prototype.displayErrorSource = function (oError) {
  alert(
    this.getMessage("errorSource", this.config.source, oError.errorDescription)
  );
};
...
Zapatec.Grid.prototype.splice = function (aData) {
  ...
  for (var iData = 0; iData < iDataLen; iData++) {
    ...
    if (
      (typeof oData.atRowId == "string" && oData.atRowId.length) ||
    ...
    if (typeof iInsertPos != "number") {
      if (
        (typeof oData.atRow == "string" && oData.atRow.length) ||
    ...
    if (typeof iInsertPos != "number") {
      ...
      // Find row number by id
      if (
        (typeof oData.afterRowId == "string" && oData.afterRowId.length) ||
    ...
    if (typeof iInsertPos != "number") {
      if (
        (typeof oData.afterRow == "string" && oData.afterRow.length) ||
...
Zapatec.Grid.prototype.sort = function (aArg) {
  ...
  if (this.order.length && this.order.length < this.fields.length) {
    ...
    for (
      var iCol = 0;
      iCol < this.fields.length && this.order.length < this.fields.length;
      iCol++
    ) {
      if (oUsed[iCol]) {
        continue;
...
Zapatec.Grid.prototype.setFilters = function () {
  ...
  for (iCol = 0; iCol < iFields; iCol++) {
    ...
    maxValue = oField.maxValue;
    if (
      aHiddenValues instanceof Array ||
      ...
      for (iRow = aFilteredRows.length - 1; iRow >= 0; iRow--) {
        ...
        if (
          aHiddenValues instanceof Array &&
          Zapatec.Utils.arrIndexOf(
            aHiddenValues,
            this.getCellValueString(oCell)
          ) >= 0
        ) {
...
Zapatec.Grid.prototype.unfilterOutColumn = function (oArg) {
  ...
  for (var iCol = 0; iCol < aCols.length; iCol++) {
    ...
    if (oField.hiddenValues instanceof Array && oField.hiddenValues.length) {
...
Zapatec.Grid.prototype.refreshCallback = function () {
  ...
  // Display totals
  if (
    typeof this.config.callbackTotalsDisplay == "function" ||
...
Zapatec.Grid.prototype.selectRow = function (oRow) {
  ...
  // Select callback
  if (
    typeof this.config.callbackCellSelect != "function" &&
...
Zapatec.Grid.prototype.unselectRow = function (oRow) {
  ...
  // Unselect callback
  if (
    typeof this.config.callbackCellUnselect != "function" &&
    ...
    for (var iRow = 0; iRow < this.rows.length; iRow++) {
      ...
      if (oCell) {
        for (var iCol = 0; iCol < this.fields.length; iCol++) {
          ...
          if (!(oCurrRow.i == iRowId && oCurrCell.i == iCellId)) {
            // Unselect cell
...
Zapatec.Grid.rowOnClick = function (iGridId, iRowId, iCellId) {
  ...
  if (oGrid && oGrid.rowOnClick) {
    ...
    console.log("renglon", iRowId);
    console.log("celda", iCellId);
  }
};
...
Zapatec.Grid.checkboxClearAllOnClick = function (iGridId, aCols) {
  ...
  if (
    !oGrid ||
    !oGrid.getColumnRange ||
    !oGrid.filterOutColumn ||
    !oGrid.applyFilters ||
    !oGrid.displayFilterOut
  ) {
    return;
    ...
    if (typeof oFilterOut.callback == "function") {
      ...
      for (var iVal = 0; iVal < aVals.length; iVal++) {
        ...
        for (var iField = 0; iField < aFields.length; iField++) {
          ...
          if (
            !(oField.hiddenValues instanceof Array) ||
...
Zapatec.Grid.prototype.totalRecords = function () {
  if (this.config.dataOnDemand && typeof this.data.totalRows != "undefined") {
...
Zapatec.Grid.prototype.setFieldTitle = function (oField, sTitle) {
  ...
  var oSpan = document.getElementById(
    ["zpGrid", this.id, "Col", oField.i, "TitleSpan"].join("")
  );
  if (oSpan) {
