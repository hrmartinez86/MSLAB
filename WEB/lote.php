...
for ($i=0;$i<count($I_Info);$i++)
{
		...
		{
			$this->SetXY($I_Info[$i]['X'],$I_Info[$i]['Y']+28);
		...
		{
			$this->SetXY($I_Info[$i]['X'],$I_Info[$i]['Y']+30);
...
function Footer()
{
	...
	for ($i=0;$i<2;$i++)
	{
		...
		{
				$this->SetXY($datos[1][$i][0],$datos[1][$i][1] + 292);
				$this->SetFont("Arial",'',$datos[1][$i][3]); // $VL_Est
				$this->Cell(0,0,$datos[1][$i][3],0,0,1,'S');
		}
		else
		{
			$this->Image($datos[1][$i][5],$datos[1][$i][6],$datos[1][$i][7]+262,$datos[1][$i][8],$datos[1][$i][9]);
		}
	}
}
}
